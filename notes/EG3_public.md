...
In-paper anchor: `paper/MOTIVIC_REALIZATIONS_PREPRINT.md` (`MRF_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of the declared motivic realization, regulator, and filtration package across admissible correspondence transport through a multi-lane motivic-realizations super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`MRF_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_MRF`.
...
If near-failure extraction and no-Zeno spacing hold, then `MRF_G3` prevents hidden drift before rigidity is applied.
